...
object Utils
{
  ...
  def getTimeValue(ts: Map[String, (TimeSeries[Any], _root_.com.github.nscala_time.time.Imports.Duration)], computeTime: LocalDateTime) : Iterable[(String,LocalDateTime,Any)] =
  {
    ts.map(series =>
    {
      val values = series._2._1

      val time = computeTime;
